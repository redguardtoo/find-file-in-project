...
For example, you can =M-x ffip-show-diff= to view the git commit and open corresponding.
...
=ffip-save-ivy-last= saves previous search result.

=ffip-ivy-resume= re-use the *saved* result by =ffip-save-ivy-last=.

You can always use =ivy-resume= to re-use previous search result.
* Tips
