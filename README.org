...
** =M-x ffip-show-diff=
Execute selected backend from =ffip-diff-backends=. The selected index is the parameter passed to =ffip-show-diff= whose default value in one.
...
The output is inserted into a new buffer "*ffip-diff*" where you can press =o=, =C-c C-c=, =ENTER= , =M-x ffip-diff-find-file=  to open the corresponding file.
...
Here are other key bindings available in =*ffip-diff*= buffer,
| key binding | command        |
|-------------+----------------|
| p           | diff-hunk-prev |
| n           | diff-hunk-next |
